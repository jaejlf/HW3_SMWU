...
void grid::initStatus(){
	...
	for (int i = 0; i < dotNum; i++){
		...
		if (beside[i][TOP] < 0){
      beside[i][TOP] = NOTHING;
		}
		if (beside[i][DOWN] >= dotNum){
			beside[i][DOWN] = NOTHING;
		}
		if (i % (n + 1) == 0){
			beside[i][LEFT] = NOTHING;
		}
		if (beside[i][RIGHT] % (n + 1) == 0){
			beside[i][RIGHT] = NOTHING;
		}
	}
