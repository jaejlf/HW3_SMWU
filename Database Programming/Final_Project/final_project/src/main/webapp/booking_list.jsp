...
<body>
	...
	String movieInfoViewSQL =
			"create or replace view MovieInfoView (movie_title, adult_only, movie_date, start_time, end_time) " +
			"as select Movie.movie_title, Movie.adult_only, Run.movie_date, Run.start_time, Run.end_time " +
			"from Running_Movie_Table Run join Movie_Info_Table Movie " +
			"on Run.movie_id = Movie.movie_id where Run.running_movie_id = ";
	...
	try {
		...
		if (bookingList != null) {
			while (bookingList.next()) {
				...
				CallableStatement cstmt_info_view = myConn.prepareCall(movieInfoViewSQL + running_movie_id);
				cstmt_info_view.execute();
				
				CallableStatement cstmt_info = myConn.prepareCall("{call get_movie_info(?,?,?,?,?)}");
				cstmt_info.registerOutParameter(1, java.sql.Types.VARCHAR);
				cstmt_info.registerOutParameter(2, java.sql.Types.CHAR);
				cstmt_info.registerOutParameter(3, java.sql.Types.DATE);
				cstmt_info.registerOutParameter(4, java.sql.Types.VARCHAR);
				cstmt_info.registerOutParameter(5, java.sql.Types.VARCHAR);
				cstmt_info.execute();
				
				String movie_title = cstmt_info.getString(1);
				String adult_only = "청소년 관람 가능";
				if (cstmt_info.getString(2) == "1") adult_only = "청소년 관람 불가";
				Date movie_date = cstmt_info.getDate(3);
				String start_time = cstmt_info.getString(4);
				String end_time = cstmt_info.getString(5);
			   	
			   	cstmt_info_view.close();
			   	cstmt_info.close();
